...
### 3.1 Independent Storage & Translation Reassembly (FSRS)
When a word is officially added to the FSRS library, the system creates an independent profile and saves it to `localStorage("customVocab")`. The writing logic must be structured as follows:
*   **For Target Words (Composite Translation)**: Combine translations using the format `[Contextual Translation] ([Master Dictionary Global Translation])`. For example: `couch potato (someone who lies on the sofa, inactive)`.
*   **For Secondary Words (Contextual Only)**: Write strictly the `[Contextual Translation]` generated by the LLM for that specific sentence.
