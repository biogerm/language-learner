...
## 4. 阶段隔离与报告协议 (Phase Isolation & Reporting Protocol)

每个阶段都必须严格遵守以下隔离和报告要求：

1. **独立的输出文件夹**：每个阶段都必须独立生成一个专属文件夹（例如 `course/sfid/phase1/`、`course/sfid/phase2/`）。该阶段产生的所有产物、中间文件和日志都必须独占地存放在这个独立的文件夹内。
2. **阶段报告文件**：在每个阶段结束时，必须在该阶段的文件夹内生成一份详细的报告文件（例如 `phase1_report.md`）。
3. **报告内容**：报告文件中必须包含详尽的统计信息以及该阶段所作出的所有修改详情，特别是改动的部分，以及任何需要着重阐述的内容。例如：
    *   **第一阶段 (Phase 1)**：所修改/修复的单词的详细列表、原始错误字符串、以及最终修正后的单词。
    *   **第二阶段 (Phase 2)**：生成的文章总数、主题分布、文章的单词分布情况、以及最终的单词覆盖率（必须为 100%）。
    *   **第三至五阶段 (Phase 3-5)**：生成数量、成功/失败率、以及部署指标等。

这份报告将作为该阶段执行情况的最终审计文件。

## 5. 执行协议 (Execution Protocol)
此流水线应由一个主 Python 脚本（例如 `build_course.py`）进行编排，该脚本严格强制执行顺序，并在进入下一阶段前验证每个阶段的输出。
