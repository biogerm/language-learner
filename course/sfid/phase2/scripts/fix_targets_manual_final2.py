            ...
            if article["article_id"] == "art_21":
                for sentence in article.get("sentences", []):
                    for target in sentence.get("target_words", []):
                        if target["word_in_sentence"] == "himla":
                            target["word_in_sentence"] = "suggestiv"
                            
            if article["article_id"] == "art_35":
                for sentence in article.get("sentences", []):
                    for target in sentence.get("target_words", []):
                        if target["word_in_sentence"] == "konditional":
                            target["word_in_sentence"] = "villkorslös"
                            
            if article["article_id"] == "art_36":
                for sentence in article.get("sentences", []):
                    if "Kommer du förresten att komma ihåg vår roliga julfest om tio år?" in sentence["sv"]:
                        sentence["sv"] = sentence["sv"].replace("om tio år?", "ännu om tio år?")
