...
def main():
    ...
    num_subagents = 3
    chunk_size = math.ceil(len(abstract_words) / num_subagents)
    
    for i in range(num_subagents):
        chunk = abstract_words[i*chunk_size : (i+1)*chunk_size]
        with open(f"abs_chunk_{i+1}.json", "w", encoding="utf-8") as f:
            json.dump(chunk, f, ensure_ascii=False, indent=2)
            
    print(f"Total abstract words: {len(abstract_words)}")
    print(f"Partitioned into 3 chunks of ~{chunk_size} words each for subagents.")
