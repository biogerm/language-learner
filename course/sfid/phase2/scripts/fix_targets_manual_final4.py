...
def fix():
    ...
    for step in data.get("steps", []):
        for article in step.get("articles", []):
            if article["article_id"] == "art_27":
                for sentence in article.get("sentences", []):
                    for target in sentence.get("target_words", []):
                        if target["word_in_sentence"] == "fylle-":
                            target["word_in_sentence"] = "fylle"
                        if target["word_in_sentence"] == "de senaste":
                            target["word_in_sentence"] = "har kunnat"
                        if target["word_in_sentence"] == "genom att":
                            target["word_in_sentence"] = "eftersom"
