...
def fix():
    ...
    for step in data.get("steps", []):
        for article in step.get("articles", []):
            if article["article_id"] == "art_52":
                for sentence in article.get("sentences", []):
                    for target in sentence.get("target_words", []):
                        if target["word_in_sentence"] == "tanke":
                            target["word_in_sentence"] = "tankar"
                            
            if article["article_id"] == "art_56":
                for sentence in article.get("sentences", []):
                    for target in sentence.get("target_words", []):
                        if target["word_in_sentence"] == "flintis /flintskallig":
                            target["word_in_sentence"] = "flintskallig"
                        if target["word_in_sentence"] == "hålla kopplad":
                            target["word_in_sentence"] = "kopplad"
