...
def run_benchmark(model_name):
    print(f"\n======================================")
    print(f"Multiprocessing Benchmark: {model_name} (5 Processes)")
    print(f"======================================")
    
    # Initialize the ProcessPool. Max workers is 5.
    with concurrent.futures.ProcessPoolExecutor(max_workers=5, initializer=init_worker, initargs=(model_name,)) as executor:
        print(f"[{model_name}] Spawning 5 processes and loading 5 model copies into RAM (please wait)...")
        
        # Warmup and force initialization across all 5 workers
        list(executor.map(process_audio, [TEST_SENTENCE_AUDIO]*5))
...
if __name__ == '__main__':
    ...
    print(f"[medium] Estimated total time (5 processes): {medium_total_mins:.2f} minutes")
    print(f"[large] Estimated total time (5 processes): {large_total_mins:.2f} minutes")
