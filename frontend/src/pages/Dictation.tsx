...
export default function Dictation() {
  ...
  const checkForNewlyExpiredWords = async () => {
    ...
    try {
      const { queue: currentDueQueue } = await buildStudyQueue('review', courseId || '', selectedArticleId, 'dictation', learningQueue);
  ...
  const handleCorrect = async () => {
    ...
    const res = await submitGatePass(courseId, currentWord.id, 'dictation', totalWrongs, totalTime, totalReveals);
    if (res.completed) {
