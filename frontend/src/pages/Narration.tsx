  ...
  return (
    <div className="glass-panel" style={{ width: '100%', maxWidth: '800px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', gap: '16px', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', flex: '1 1 auto', minWidth: '200px' }}>
          <select className="module-selector" value={selectedStage} onChange={handleStageChange} style={{ flex: '1 1 auto', maxWidth: '100%' }}>
            {stages.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
          </select>
          <select className="module-selector" value={selectedArticleId} onChange={e => setSelectedArticleId(e.target.value)} style={{ flex: '1 1 auto', maxWidth: '100%' }}>
